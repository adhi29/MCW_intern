...
"""
End-to-end comparison: Original Alpamayo PyTorch vs Static FP32 ONNX pipeline.

Handles:
  - fp32 model I/O (inputs/outputs are float16)
  - Static KV cache for decoder_decode (padded to 3262)
  - Static shapes throughout (no dynamic axes)

Usage:
    python compare_e2e_static.py
    python compare_e2e_static.py --onnx-dir ./onnx_models_static
"""
...
def static_fp16_onnx_inference(
    ...
    for step in range(max_generation_length):
        ...
        if actual_seq < MAX_TOTAL_SEQ:
            ...
            padded_keys_np = np.concatenate(
                [real_past_keys_np, np.zeros(pad_shape_k, dtype=np.float32)], axis=3
            )
            padded_values_np = np.concatenate(
                [real_past_values_np, np.zeros(pad_shape_k, dtype=np.float32)], axis=3
            )
        else:
        ...
        decode_attn_mask_np = np.full(
            (B, 1, 1, total_mask_len), np.finfo(np.float32).min, dtype=np.float32
        )
    ...
    if actual_seq < MAX_TOTAL_SEQ:
        ...
        diff_past_keys_np = np.concatenate(
            [real_past_keys_np, np.zeros(pad_shape, dtype=np.float32)], axis=3
        )
        diff_past_values_np = np.concatenate(
            [real_past_values_np, np.zeros(pad_shape, dtype=np.float32)], axis=3
        )
    else:
    ...
    diff_attention_mask_np = np.full(
        (b_star, 1, n_diffusion_tokens, diff_total_len), np.finfo(np.float32).min, dtype=np.float32
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Compare PyTorch vs Static FP32 ONNX inference"
    )
    parser.add_argument("--onnx-dir", type=str, default="./onnx_models_static")
    parser.add_argument(
    ...
    print("=" * 80)
    print("END-TO-END COMPARISON: PyTorch vs Static FP32 ONNX")
    print("=" * 80)
    ...
    # ==========================================
    # 2. Run Static FP32 ONNX pipeline
    # ==========================================
    print("\n" + "-" * 60)
    ...
    print(f"\n{'':>25s} {'PyTorch':>15s}  {'Static FP32 ONNX':>18s}")
    ...
    save_path = os.path.join(args.save_dir, f"compare_fp32_{args.clip_id[:8]}.png")
    ...
    for i in range(onnx_pred_xy_all.shape[0]):
        ...
        ax.plot(*xy_rot, "b-", linewidth=1.5, alpha=0.8,
                label="Static FP32 ONNX" if i == 0 else "_nolegend_")
